...
- **v2.0.0（当前）**：新增「分享链接一条龙」（解析别人分享的 139 链接、自动识别提取码、一键转存视频到自己的云盘），并支持**转存后自动生成 CAS**、**生成后删除原视频**两个可选复选框，一条龙省一步；新增视频后缀白名单（只转换 MP4/MKV 等视频，跳过图片/文本）；修复重复生成 CAS（`auto_rename`→`overwrite`）与 `.cas` 后缀叠加问题；登录态持久化（刷新页面自动重登）；**去掉强制加密依赖**——`pycryptodome` 改为可选（仅 `OutLinkCrypto` 备用类按需惰性导入），主流程零额外依赖、Docker 镜像更轻、三架构构建更稳。
