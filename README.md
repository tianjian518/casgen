...
1. **本程序主流程只用 Python 标准库，仅新增一个加密依赖 `pycryptodome`。** 只要设备里有 Python3 就能跑；用 Docker 则连 Python 都不用装（镜像已内置依赖）。本地直接跑的话，先 `pip install pycryptodome` 再启动即可。
...
4. 之后回到「选文件夹」选你刚转存的目录，按上面的步骤生成 `.cas` 瘦身即可。

> 说明：分享解析走的是移动云盘 139 的明文 V6 接口（无需额外签名）；`share139.py` 里另带一个 `OutLinkCrypto` 加密外链备用类（基于 pycryptodome 的 AES-128-CBC），供后续需要时解析官方 App 的加密外链。
...
- **v2.0.0（当前）**：新增「分享链接一条龙」（解析别人分享的 139 链接、自动识别提取码、一键转存视频到自己的云盘）；新增视频后缀白名单（只转换 MP4/MKV 等视频，跳过图片/文本）；修复重复生成 CAS（`auto_rename`→`overwrite`）与 `.cas` 后缀叠加问题；登录态持久化（刷新页面自动重登）；引入加密依赖 `pycryptodome`（用于备用加密外链解析，Docker 镜像已内置）。
